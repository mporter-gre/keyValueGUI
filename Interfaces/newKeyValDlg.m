...
cancelled = getappdata(handles.newKeyValDlg, 'cancelled');
[numVals, ~] = size(newVal);
...
if ~isempty(newKey) && ~isempty(newVal)
    ...
    warndlg('A Key and at least one Value are required', 'Missing entries');
    return;
end
...
setappdata(handles.newKeyValDlg, 'cancelled', 1);
uiresume(handles.newKeyValDlg);
